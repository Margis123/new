    ...
    class MS_BASE_ADDONS
    {
		...
		public function show_settings()
		{
			...
			if(!empty($available_addons))
			{
				...
				foreach($available_addons as $addon)
				{
					if(class_exists($addon->class)) continue;
					if(
...
?>
					<div class="postbox ms-webhook-addon">
						<h3 class='hndle' style="padding:5px;"><span><?php _e($addon->title, MS_TEXT_DOMAIN); ?></span></h3>
						<div class="inside">
							<p><?php _e($addon->description,MS_TEXT_DOMAIN); ?></p>
							<div style="text-align:right;">
							<?php
								if($is_installed)
								{
									print '<input type="button" value="'.esc_attr__('Activate add on', MS_TEXT_DOMAIN).'" onclick="document.location.href=\''.esc_js(admin_url('plugins.php')).'\'" class="button-primary" />';
								}
								else
								{
									if(stripos($addon->link, 'https://downloads.wordpress.org/') !== false)
									{
										print '<input type="submit" value="'.esc_attr__('Install add on', MS_TEXT_DOMAIN).'" class="button-primary" onclick="'.esc_attr('jQuery(this).after(\'<input type="hidden" name="ms-addon-install" value="'.esc_attr($addon->name).'">\')').'" />';
									}
									else
									{
										print '<input type="button" value="'.esc_attr__('Install add on', MS_TEXT_DOMAIN).'" onclick="document.location.href=\''.esc_js($addon->link).'\'" class="button-primary" />';
									}
								}
							?>
							</div>
