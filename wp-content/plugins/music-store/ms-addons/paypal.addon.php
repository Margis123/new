	...
	class MUSIC_STORE_PAYPAL_ADDON
	{
		...
		public function populate_list($payment_gateways)
		{
			if($this->paypal_enabled(false))
				$payment_gateways['paypal'] = __('PayPal', MS_TEXT_DOMAIN);
		...
		public function send_to_paypal($amount, $purchase_settings)
		{
			...
			if(
				...
				$_REQUEST['ms_payment_gateway'] != 'paypal'
			) return;
			...
			if( !empty( $ms_paypal_email ) ) // Check for sealer email
			{
				if($amount > 0)
				{
				...
				?>
					...
					<input type="hidden" name="item_name" value="<?php print esc_attr($item_name); ?>" />
					<input type="hidden" name="item_number" value="Item Number <?php print esc_attr($item_number); ?>" />
					...
					<input type="hidden" name="notify_url" value="<?php print esc_url(apply_filters('musicstore_notify_url', $baseurl.'|pg=paypal|pid='.$products_id.'|purchase_id='.$purchase_id.'|rtn_act=purchased_product_music_store'.$coupon_code)); ?>" />
		...
		{
			global $music_store_settings;
			if(!$this->paypal_enabled(false)) return;
			...
			if(!empty($_GET['ms-action']))
			{
				...
				if(!empty($ipn_parameters['pg']) && $ipn_parameters['pg'] != 'paypal') return;

				if(!isset($ipn_parameters['purchase_id'])) exit;
				$purchase_id = $ipn_parameters['purchase_id'];

				$item_name 			= (!empty($_POST['item_name'])) ? sanitize_text_field($_POST['item_name']) : '';
				// $item_number 		= (!empty($_POST['item_number'])) ? $_POST['item_number'] : '';
				$payment_status 	= (!empty($_POST['payment_status'])) ? sanitize_text_field($_POST['payment_status']) : '';
				$payment_amount 	= (!empty($_POST['mc_gross'])) ? @floatval($_POST['mc_gross']) : 0;
				$tax 				= 0;
				if(!empty($_POST['tax'])) $tax = @floatval($_POST[ 'tax' ]);
				$payment_amount -= $tax;
				$payment_currency 	= (!empty($_POST['mc_currency'])) ? sanitize_text_field($_POST['mc_currency']) : 'USD';
				// $txn_id 			= (!empty($_POST['txn_id'])) ? $_POST['txn_id'] : '';
				// $receiver_email 	= (!empty($_POST['receiver_email'])) ? $_POST['receiver_email'] : '';
				$payer_email 		= (!empty($_POST['payer_email'])) ? sanitize_email($_POST['payer_email']) : '';
				...
				$paypal_data = "";
				foreach ($_POST as $item => $value) $paypal_data .= sanitize_key($item)."=".sanitize_text_field($value)."\r\n";
				...
				{
					$coupon = $wpdb->get_row(
						$wpdb->prepare("SELECT * FROM ".$wpdb->prefix.MSDB_COUPON." WHERE coupon=%s", $ipn_parameters['ms_coupon_code'])
					);
				...
				if(!empty($music_store_settings['ms_paypal_shopping_cart']))
				{
					...
					foreach($products as $product)
					{
						...
						else
						{
							if(
								...
								$product->price <= ($variable_price = floatval($product->price_type))
							)
							{
					...
					if($payment_amount - $total < -0.5) exit;
				...
				else
				{
					...
					if(
						...
						$ipn_parameters[ 'price_type' ] == 'exclusive' &&
					...
					else
					{
						if(
							...
							$obj->price <= ($variable_price = floatval($obj->price_type))
						)
						{
					...
					if( $payment_amount - $price < -0.5 ) exit;
					if(
						music_store_register_purchase(
							$id,
							$purchase_id,
				...
				do_action('musicstore_payment_received', $_POST, (isset($products)) ? $products : $obj);
				echo 'OK';
				exit;
			}
		} // End check_payment
