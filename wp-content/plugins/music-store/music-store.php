		...
		*/
			...
			if ( ! is_admin()){
                ...
                if(isset($_REQUEST['ms-action'])){
                    switch(strtolower($_REQUEST['ms-action'])){
						...
						break;
						case 'plays':
							if(!empty($_POST['id']) && ($_product_id = @intval($_POST['id'])) != 0)
							{
								if( !isset( $_SESSION[ 'ms-plays-registered' ] ) ) $_SESSION[ 'ms-plays-registered' ] = array();
								if( !in_array( $_product_id, $_SESSION[ 'ms-plays-registered' ] ) )
								{
									$wpdb->query($wpdb->prepare('UPDATE '.$wpdb->prefix.'msdb_post_data SET plays=plays+1 WHERE id=%d', $_product_id));
									$_SESSION[ 'ms-plays-registered' ][] = $_product_id;
								}
							}
							exit;
						break;
							...
							if(stripos($_REQUEST['ms-action'],'ipn|') !== false)
							{
								if($music_store_settings['ms_debug_payment'])
								{
									try
									{
										...
										@error_log('Music Store payment gateway POST parameters: '.json_encode($_POST));
									}
									catch(Exception $err){}
								}
		...
		function _preview()
		{
			...
			if(array_intersect($allowed_roles, $user->roles ))
			{
				...
				{
					// Sanitizing variable
					$preview = stripcslashes($_REQUEST['ms-preview']);
					...
					if(has_shortcode($preview, 'music_store'))
					{
						$scale = true;
						$plus = '+25';

						$if_empty = __('There are no products that satisfy the block\'s settings', MS_TEXT_DOMAIN);
...
/** CODE REQUIRED FOR DOWNLOAD PAGE **/
		...
		{
			if($post->post_name == 'ms-download-page')
				$post_states['ms-download-page'] = 	'Music Store - '.__('Download Page', MS_TEXT_DOMAIN);
			...
			if( isset( $_REQUEST ) && isset( $_REQUEST[ 'ms-action' ] ) && strtolower( $_REQUEST[ 'ms-action' ] ) == 'download' ){
				...
				if( empty( $ms_errors ) ){
					$_download_links = apply_filters(
							...
							.__('Download Links:', MS_TEXT_DOMAIN)
							.'</div><div>'
		...
		function admin_init(){
			...
			if( isset( $_REQUEST[ 'ms-action' ] ) && $_REQUEST[ 'ms-action' ] == 'paypal-data' ){
				if( isset( $_REQUEST[ 'data' ] ) && isset( $_REQUEST[ 'from' ] ) && isset( $_REQUEST[ 'to' ] ) ){
					$where = 'DATEDIFF(date, "'.sanitize_text_field($_REQUEST[ 'from' ]).'")>=0 AND DATEDIFF(date, "'.sanitize_text_field($_REQUEST[ 'to' ]).'")<=0';
...
/** READ THE STORE SETTINGS **/
		...
		{
			global $music_store_settings;
			if($music_store_settings['ms_notification_from_email'] == MS_NOTIFICATION_FROM_EMAIL)
			{
				...
				if ($pos === false) $music_store_settings['ms_notification_from_email'] = 'admin@'.$domain;
			}

			if($music_store_settings['ms_notification_to_email'] == MS_NOTIFICATION_TO_EMAIL)
			{
				if(!isset($user_email)) $user_email = get_the_author_meta('user_email', get_current_user_id());
				if(!empty($user_email)) $music_store_settings['ms_notification_to_email'] = $user_email;
			}
...
/** MANAGE DATABASES FOR ADITIONAL POST DATA **/
		...
		*/
		...
		function redirect_to_settings($plugin, $network_activation)
		{
			...
			{
				exit( wp_redirect( admin_url( 'admin.php?page=music-store-menu-settings' ) ) );
			}
		}
		...
		*/
		private function _create_db_structure( $installing = false ){
            try{
				/* if( !$installing && !empty( $_SESSION[ 'msdb_created_db' ] ) ) return;
		...
		*/
		function menu_links(){
			if(is_admin()){
				...
				// Settings Submenu
				add_submenu_page($this->music_store_slug, __( 'Music Store Settings', MS_TEXT_DOMAIN ), __( 'Store Settings', MS_TEXT_DOMAIN ), 'manage_options', $this->music_store_slug.'-settings', array(&$this, 'settings_page'));

				// Sales report submenu
				add_submenu_page($this->music_store_slug, __( 'Music Store Sales Report', MS_TEXT_DOMAIN ), __( 'Sales Report', MS_TEXT_DOMAIN ), 'manage_options', $this->music_store_slug.'-reports', array(&$this, 'settings_page'));

				// Importer submenu
				add_submenu_page($this->music_store_slug, __( 'Songs Importer', MS_TEXT_DOMAIN ), __( 'Songs Importer', MS_TEXT_DOMAIN ), 'manage_options', $this->music_store_slug.'-importer', array(&$this, 'importer'));
		...
		*/
		function settings_page(){
			...
			if ( isset( $_POST['ms_settings'] ) && wp_verify_nonce( $_POST['ms_settings'], plugin_basename( __FILE__ ) ) )
			{
				...
				update_option('ms_pp_cover_size', (isset($_POST['ms_pp_cover_size']) && $_POST['ms_pp_cover_size'] == 'full') ? 'full' : 'medium');

				update_option('ms_pp_related_products_number', (empty($_POST['ms_pp_related_products_number']) || ($ms_pp_related_products_number = @intval($_POST['ms_pp_related_products_number'])) == 0) ? 3 : $ms_pp_related_products_number);

				update_option('ms_pp_related_products_columns', (empty($_POST['ms_pp_related_products_columns']) || ($ms_pp_related_products_columns = @intval($_POST['ms_pp_related_products_columns'])) == 0) ? 3 : $ms_pp_related_products_columns);

				update_option('ms_pp_related_products_by', (empty($_POST['ms_pp_related_products_by'])) ? array() : ((is_array($_POST['ms_pp_related_products_by'])) ? array_map('sanitize_text_field', $_POST['ms_pp_related_products_by']): sanitize_text_field($_POST['ms_pp_related_products_by'])));
				...
				update_option('ms_paypal_currency', sanitize_text_field(html_entity_decode($_POST['ms_paypal_currency'])));
				update_option('ms_paypal_currency_symbol', sanitize_text_field(html_entity_decode($_POST['ms_paypal_currency_symbol'])));
				update_option('ms_paypal_language', sanitize_text_field(html_entity_decode($_POST['ms_paypal_language'])));
				...
				update_option('ms_tax', ((!empty($_POST['ms_tax']) && ($ms_tax = trim($_POST['ms_tax'])) != '') ? @floatval($ms_tax) : false));
				...
				update_option('ms_facebook_app_id', ((!empty($_POST['ms_facebook_app_id'])) ? sanitize_text_field($_POST['ms_facebook_app_id']) : ''));
				...
				{
					$ms_first_time_mssg = '<span id="ms_first_time_mssg">'.__('Settings Updated', MS_TEXT_DOMAIN).' - </span>';
...
?>
					...
					<div class="postbox">
						...
						<div class="inside">
							...
							<table class="form-table">
								...
								<tr valign="top">
									<th><?php _e('Show the products popularity', MS_TEXT_DOMAIN); ?></th>
									<td>
										<input type="checkbox" name="ms_popularity" id="ms_popularity" <?php if($music_store_settings['ms_popularity']) print 'CHECKED'; ?> />
									</td>
								</tr>
								<tr valign="top">
					...
					<div class="postbox">
						<h3 class='hndle' style="padding:5px;"><span><?php _e('Product config', MS_TEXT_DOMAIN); ?></span></h3>
						<div class="inside">
							<?php
							...
							<table class="form-table">
								<tr valign="top">
									<th><?php _e('Size of cover image', MS_TEXT_DOMAIN); ?></th>
									<td>
										<input type="radio" name="ms_pp_cover_size" <?php if( empty($music_store_settings[ 'ms_pp_cover_size' ]) || $music_store_settings[ 'ms_pp_cover_size' ] == 'medium') echo 'checked'; ?> value="medium" /> <?php _e('Medium size', MS_TEXT_DOMAIN); ?>
										<input type="radio" name="ms_pp_cover_size" <?php if( !empty($music_store_settings[ 'ms_pp_cover_size' ]) && $music_store_settings[ 'ms_pp_cover_size' ] == 'full') echo 'checked'; ?> value="full" /> <?php _e('Full size', MS_TEXT_DOMAIN); ?><br />
										<i><?php _e( 'The size of cover image selected only affects to the images associated to the products from now, the images selected previously won\'t be modified' , MS_TEXT_DOMAIN); ?></i>
									</td>
								</tr>
								<tr>
									<td colspan="2"><h3><?php _e('Product page', MS_TEXT_DOMAIN); ?></h3></td>
								</tr>
								<tr valign="top">
									<th><?php _e('Include list of related products', MS_TEXT_DOMAIN); ?></th>
									<td>
										<input type="checkbox" name="ms_pp_related_products" <?php if($music_store_settings[ 'ms_pp_related_products' ]) echo 'checked'; ?> />
									</td>
								</tr>
								<tr valign="top">
									<th><?php _e('Number of related products', MS_TEXT_DOMAIN); ?></th>
									<td>
								...
								<tr valign="top">
									<th><?php _e('Display list in', MS_TEXT_DOMAIN); ?></th>
									<td><input type="number" name="ms_pp_related_products_columns" value="<?php echo esc_attr( $music_store_settings[ 'ms_pp_related_products_columns' ] ); ?>" /> <?php _e('columns', MS_TEXT_DOMAIN); ?>
									</td>
								</tr>
								<tr valign="top">
									<th><?php _e('Products related by', MS_TEXT_DOMAIN); ?></th>
									<td>
										<input type="checkbox" name="ms_pp_related_products_by[]" value="album" <?php if (in_array('album', $music_store_settings[ 'ms_pp_related_products_by' ])) echo 'checked'; ?> />
										<?php _e('Albums', MS_TEXT_DOMAIN); ?><br>
										<input type="checkbox" name="ms_pp_related_products_by[]" value="genre" <?php if (in_array('genre', $music_store_settings[ 'ms_pp_related_products_by' ])) echo 'checked'; ?> />
										<?php _e('Genres', MS_TEXT_DOMAIN); ?><br>
										<input type="checkbox" name="ms_pp_related_products_by[]" value="artist" <?php if (in_array('artist', $music_store_settings[ 'ms_pp_related_products_by' ])) echo 'checked'; ?> />
										<?php _e('Artists', MS_TEXT_DOMAIN); ?><br>
									</td>
								</tr>
							</table>
							<?php
                    ...
                    <!-- PAYPAL BOX -->
					<div id="ms_ipn_video_tutorial" <?php print $ms_video_style; ?>>
						<div style="padding:0 10px 10px 0;"><span><?php print $ms_first_time_mssg; _e('How configure your PayPal account', MS_TEXT_DOMAIN); ?></span><span style="float:right;"><a href="javascript:void(0);" onclick="jQuery('#ms_ipn_video_tutorial').hide();"><?php _e('close [x]', MS_TEXT_DOMAIN);?></a></span></div>
						<video controls preload="none" width="100%">
							<source src="https://musicstore.dwbooster.com/videos/ipn.mp4" type="video/mp4">
						</video>
					</div>
					...
					<div class="postbox">
						<h3 class='hndle' style="padding:5px;"><span><?php _e('Payment Gateway Configuration', MS_TEXT_DOMAIN); ?></span></h3>
						<div class="inside">
						<?php
							...
							{
								print '<div style="border: 1px solid #FF0000; background-color: rgba(255,0,0,0.1); padding: 10px; margin: 10px 0;font-weight:bold;">'.__('Your website is hosted locally, so, the PayPal IPN cannot reach your website. For testing the purchases the website should be hosted publicly.', MS_TEXT_DOMAIN).'</div>';
							}
						?>
						<table class="form-table">
							<tr valign="top">
							<th scope="row"><?php _e('Enable Paypal Payments?', MS_TEXT_DOMAIN); ?></th>
							<td><input type="checkbox" name="ms_paypal_enabled" value="1" <?php if ($music_store_settings[ 'ms_paypal_enabled' ]) echo 'checked'; ?> /><br><i>Remember to enable the IPN (Instant Payments Notification) in the PayPal account (use the URL to your home page in the process): <a href="https://developer.paypal.com/docs/classic/ipn/integration-guide/IPNSetup/#id089EG030E5Z" target="_blank">How to enable the IPN?</a></i></td>
							</tr>

							<tr valign="top">
							<th scope="row"><?php _e('Use Paypal Sandbox', MS_TEXT_DOMAIN); ?></th>
							<td><input type="checkbox" name="ms_paypal_sandbox" value="1" <?php if ($music_store_settings[ 'ms_paypal_sandbox' ] ) echo 'checked'; ?> /><br><i>The PayPal Sandbox account and the PayPal account are independent, remember to enable the IPN in the PayPal Sandbox account too: <a href="https://developer.paypal.com/docs/classic/ipn/integration-guide/IPNSetup/#id089EG030E5Z" target="_blank">How to enable the IPN?</a></i></td>
							</tr>
                            ...
                            <span class="ms_more_info_hndl" style="margin-left: 10px;"><a href="javascript:void(0);" onclick="ms_display_more_info( this );">[ + <?php _e('more information', MS_TEXT_DOMAIN); ?>]</a></span><span style="margin-left: 10px;"><a href="javascript:void(0);" onclick="jQuery('#ms_first_time_mssg').hide();jQuery('#ms_ipn_video_tutorial').show();">[ + <?php _e('enabling the IPN in PayPal tutorial', MS_TEXT_DOMAIN); ?>]</a></span>
							...
							<tr valign="top">
							<th scope="row"><?php _e('Payment button', MS_TEXT_DOMAIN); ?></th>
							...
							<tr valign="top">
							<th scope="row"><?php _e('Hide download link if price in blank', MS_TEXT_DOMAIN); ?></th>
							<td><input type="checkbox" name="ms_hide_download_link_for_price_in_blank" <?php if(!empty($music_store_settings['ms_hide_download_link_for_price_in_blank'])) print 'CHECKED'; ?> /><i><?php _e('The plugin assumes the songs whose price is left in blank are distributed for free, and includes a download link in them, click the checkbox to change this behavior.', MS_TEXT_DOMAIN); ?></i></td>
							</tr>

							<tr><td colspan="2"><hr /></td></tr>

							<tr valign="top">
							<th scope="row"><?php _e('Apply taxes (in percentage)', MS_TEXT_DOMAIN); ?></th>
							<td><input type="number" name="ms_tax" value="<?php if(!empty($music_store_settings['ms_tax'])) print esc_attr($music_store_settings['ms_tax']); ?>" /></td>
							</tr>
							...
							<tr>
								<td colspan="2">
									...
									<input type="checkbox" name="ms_debug_payment" <?php print $music_store_settings['ms_debug_payment'] ? 'CHECKED' : ''; ?> /> <b><?php _e('Debugging Payment Process', MS_TEXT_DOMAIN); ?></b><br /><br />
									<i><?php _e("(If the checkbox is ticked the plugin will create two new entries in the error  logs file on your server, with the texts <b>Music Store payment gateway GET parameters</b> and <b>Music Store payment gateway POST parameters</b>.  If after a purchase, none of these entries appear in the error logs file, the payment notification has not reached the plugin's code)", MS_TEXT_DOMAIN); ?></i>
									</div>
								</td>
							</tr>
							<tr><td colspan="2"><hr /></td></tr>
					...
					<div class="postbox">
						...
						<table class="form-table">
							...
							<i><?php print __('EX',MS_TEXT_DOMAIN).': admin@'.str_replace('www.','',$_SERVER["HTTP_HOST"]); ?></i>
							</td>
							</tr>
...
<?php
				...
				case 'reports':
					...
					if ( isset($_POST['ms_purchase_stats']) && wp_verify_nonce( $_POST['ms_purchase_stats'], plugin_basename( __FILE__ ) ) ){
						if(isset($_POST['ms_new_entry'])) // Create a new entry in the sales reports
						{
								...
								if(!empty($_POST['new_entry_product']))
								{
									if(!empty($_POST['new_entry_year']))
									{
										...
										)
										{
											...
											{
												$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix.MSDB_POST_DATA." SET purchases=purchases+1 WHERE id=%d", @intval($_POST['new_entry_product'])));

												$message .= __('New entry added to the sales report',MS_TEXT_DOMAIN);
											}
											else
											{
												$error_message .= '<li>'.__('The new entry couldn\'t be inserted in the sales reports',MS_TEXT_DOMAIN).'</li>';
											}
										}
										else
										{
											$error_message .= '<li>'.__('The amount and currency are required',MS_TEXT_DOMAIN).'</li>';
										}
									}
									else
									{
										$error_message .= '<li>'.__('The date is wrong',MS_TEXT_DOMAIN).'</li>';
									}
								}
								else
								{
									$error_message .= '<li>'.__('The product is required',MS_TEXT_DOMAIN).'</li>';
								}
							}
							else
							{
								$error_message .= '<li>'.__('The buyer is required',MS_TEXT_DOMAIN).'</li>';
							}
						}
						...
						if(isset($_POST['show_purchase_id'])){ // Display paypal details
							$paypal_data = '<div class="ms-paypal-data"><h3>' . __( 'Payment data', MS_TEXT_DOMAIN ) . '</h3>' . $wpdb->get_var($wpdb->prepare(
					...
					$buyer = ( !empty( $_POST['buyer'] ) ) ? sanitize_email($_POST[ 'buyer' ]) : '';
					...
					$group_by = (isset($_POST['group_by'])) ? sanitize_text_field($_POST['group_by']) : 'no_group';
					$to_display = (isset($_POST['to_display'])) ? sanitize_text_field($_POST['to_display']) : 'sales';
...
?>
					...
					<!-- MANUAL ENTRY -->
					<div class="postbox">
						<h3 class='hndle' style="padding:5px;"><span><?php _e('Manual Entry', MS_TEXT_DOMAIN); ?></span></h3>
						<div class="inside">
							<div>
								<?php
								if(!empty($error_message))
								{
									print '<div class="music-store-error-mssg"><ul>'.$error_message.'</ul></div>';
								}
								if(!empty($message))
								{
									print '<div class="music-store-mssg"><ul>'.$message.'</ul></div>';
								}
								?>
								<table>
									<tr>
										<td>
											<label><?php _e('Buyer', MS_TEXT_DOMAIN); ?>*:</label>
										</td>
										<td>
									...
									<tr>
										<td>
											<label><?php _e('Product', MS_TEXT_DOMAIN); ?>*:</label>
										</td>
										<td>
											<select name="new_entry_product" id="new_entry_product">
												<option value=""><?php _e('Select a product', MS_TEXT_DOMAIN); ?></option>
												<?php
													...
													if($all_products)
													{
														...
														{
															print '<option value="'.esc_attr($product['ID']).'">'.esc_html('('.$product['ID'].') '.$product['post_title']).'</option>';
														}
													}
												?>
											</select>
									...
									<tr>
										<td>
											<label><?php _e('Date', MS_TEXT_DOMAIN); ?>*:</label>
										</td>
										<td>
									...
									<tr>
										<td>
											<label><?php _e('Amount', MS_TEXT_DOMAIN); ?>*: </label>
										</td>
										<td>
											<input type="text" name="new_entry_amount" />
											<label><?php _e('Currency', MS_TEXT_DOMAIN); ?>*: </label>
											<input type="text" name="new_entry_currency" placeholder="<?php _e('for example: USD', MS_TEXT_DOMAIN); ?>" />
										</td>
									</tr>
									<tr>
										<td valign="top">
											<label><?php _e('Payment data'); ?>: </label>
										</td>
										<td>
								...
								<input type="submit" value="<?php esc_attr_e(__('Add Entry', MS_TEXT_DOMAIN)); ?>" class="button-primary" onclick="jQuery(this).closest('form').append('<input type=\'hidden\' name=\'ms_new_entry\' value=\'1\' />');" />
							</div>
...
/** LOADING PUBLIC OR ADMINSITRATION RESOURCES **/
		...
		*/
		function public_resources(){
			...
			wp_enqueue_style('ms-style', plugin_dir_url(__FILE__).'ms-styles/ms-public.css', array( 'wp-mediaelement' ) );

			wp_enqueue_script('jquery');
			wp_enqueue_script('wp-mediaelement');

			wp_enqueue_script('ms-media-script', plugin_dir_url(__FILE__).'ms-script/codepeople-plugins.js', array('wp-mediaelement'));
		...
		function admin_resources($hook){
			...
			if ( $hook == 'post-new.php' || $hook == 'post.php' || $hook == 'index.php') {
				...
				if(isset($post) && $post->post_type == "ms_song"){
					...
					wp_enqueue_style('ms-admin-style', plugin_dir_url(__FILE__).'ms-styles/ms-admin.css');
					wp_localize_script('ms-admin-script', 'music_store', array('post_id'  	=> $post->ID, 'cover' => $music_store_settings['ms_pp_cover_size']));
				}else{
...
/** LOADING MUSIC STORE AND ITEMS ON WORDPRESS SECTIONS **/
		 ...
		 *
		 */
		function populate_payment_gateways_list( $tags )
		{
			global $music_store_settings;
			...
			elseif(1 < $count)
			{
				$list .= '<span class="ms-payment-gateway-label">'.__('Payment method', MS_TEXT_DOMAIN).': </span><span class="ms-payment_gateway"><select name="ms_payment_gateway">';
		...
		*/
		function load_store($atts, $content, $tag){
			...
			if(isset($_REQUEST['filter_by_genre'])){
				$_SESSION[ $page_id ]['ms_genre'] = sanitize_text_field($_REQUEST['filter_by_genre']);
			}

            if(isset($_REQUEST['filter_by_album'])){
				$_SESSION[ $page_id ]['ms_album'] = sanitize_text_field($_REQUEST['filter_by_album']);
			}

			if(isset($_REQUEST['filter_by_artist'])){
				$_SESSION[ $page_id ]['ms_artist'] = sanitize_text_field($_REQUEST['filter_by_artist']);
			}
			...
			$_order_by 	= "ORDER BY ".(($_SESSION[ $page_id ]['ms_ordering'] == "post_title" || $_SESSION[ $page_id ]['ms_ordering'] == "post_date") ? "posts" : "posts_data").".".
			(
				(
					$_SESSION[ $page_id ]['ms_ordering'] == 'price_high_low' ||
					$_SESSION[ $page_id ]['ms_ordering'] == 'price_low_high'
				) ? 'price' : $_SESSION[ $page_id ]['ms_ordering']
			)." ".(($_SESSION[ $page_id ]['ms_ordering'] == "popularity" || $_SESSION[ $page_id ]['ms_ordering'] == "post_date" || $_SESSION[ $page_id ]['ms_ordering'] == "price_high_low") ? "DESC" : "ASC");
			...
			foreach( $_GET as $var => $value )
            {
                ...
                {
                    $header .= "<input type='hidden' name='".esc_attr($var)."' value='".esc_attr(sanitize_text_field($value))."' />";
                }
            }
			...
			if(
				...
				if($allow_filter_by_genre){
					...
					if(!is_array($genre)) $genre = array($genre);
				...
				if( !isset( $atts[ 'show_order_by' ] ) || $atts[ 'show_order_by' ] * 1 )
				{
					$header .= "<div class='music-store-ordering'>".
									...
									"<select id='ordering_by' name='ordering_by' onchange='this.form.submit();'>
										<option value='post_date' ".(($_SESSION[ $page_id ]['ms_ordering'] == 'post_date') ? "SELECTED" : "").">".__('Newest', MS_TEXT_DOMAIN)."</option>
										<option value='post_title' ".(($_SESSION[ $page_id ]['ms_ordering'] == 'post_title') ? "SELECTED" : "").">".__('Title', MS_TEXT_DOMAIN)."</option>
										<option value='popularity' ".(($_SESSION[ $page_id ]['ms_ordering'] == 'popularity') ? "SELECTED" : "").">".__('Popularity', MS_TEXT_DOMAIN)."</option>
										<option value='price_low_high' ".(($_SESSION[ $page_id ]['ms_ordering'] == 'price_low_high') ? "SELECTED" : "").">".__('Price: Low to High', MS_TEXT_DOMAIN)."</option>
										<option value='price_high_low' ".(($_SESSION[ $page_id ]['ms_ordering'] == 'price_high_low') ? "SELECTED" : "").">".__('Price: High to Low', MS_TEXT_DOMAIN)."</option>
									</select>
								</div>";
				}
		...
		*
		*/
		function load_product_list($atts){
            global $wpdb;
			...
			if(!empty($exclude)) $args['post__not_in'] = explode(',', $exclude);

			switch( $type )
			{
				case 'top_rated':
					$args['orderby'] = array( 'popularity' => 'DESC', 'post_date' => 'DESC');
				break;
				case 'top_selling':
					$args['orderby'] = array( 'purchases' => 'DESC', 'post_date' => 'DESC');
				break;
				default:
			...
			if($results->have_posts())
			{
				...
				while($results->have_posts())
				{
					...
					$products_list .= "<div style='width:{$width}%;' class='music-store-item'>".$obj->display_content('store', $tpl, 'return')."</div>";
					$item_counter++;
					if($item_counter % $columns == 0)
...
/** MODIFY CONTENT OF POSTS LOADED **/
	...
	if( !defined('DOING_AJAX') && !isset($_GET['context']) && session_id() == "" ) @session_start();
