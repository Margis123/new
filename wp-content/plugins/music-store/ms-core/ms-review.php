...
if(!defined('MSDB_REVIEWS')) define( 'MSDB_REVIEWS', 'msdb_reviews');
	...
	class MS_REVIEW
	{
		...
		static public function set_review($id, $review)
		{
			...
			if(function_exists('ms_getIP'))
			{
				...
				$wpdb->query(
					$wpdb->prepare(
						"INSERT INTO ".$wpdb->prefix.MSDB_REVIEWS.
		...
		static public function get_review($id)
		{
			...
			return $wpdb->get_row(
				$wpdb->prepare(
					"SELECT FLOOR(AVG(review)) as average, COUNT(review) as votes FROM ".$wpdb->prefix.MSDB_REVIEWS.
					" WHERE product=%d",
					$id
				),
				ARRAY_A
