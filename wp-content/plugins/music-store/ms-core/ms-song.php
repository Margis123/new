		...
		*/
		...
		function display_content($mode, $tpl_engine, $output='echo'){
			...
			$song_arr = array(
				...
				'popularity' => ($music_store_settings['ms_popularity']) ? (($reviews) ? @intval($reviews['average']) : 0) : null,
				'votes'		 => apply_filters('musicstore_song_votes', ($reviews) ? @intval($reviews['votes']) : 0, $this->id),
				'cover' => null,
                'social' => null,
			...
			if(!empty($this->cover)) $song_arr['cover'] = apply_filters('musicstore_song_cover_url', esc_url( $this->cover ), $this->id);
            if($this->time) $song_arr['time'] = apply_filters('musicstore_song_time', strip_tags(html_entity_decode($this->time)), $this->id);
			if($this->year) $song_arr['year'] = apply_filters('musicstore_song_year', @intval($this->year), $this->id);
			if($this->isrc) $song_arr['isrc'] = apply_filters('musicstore_song_isrc', strip_tags(html_entity_decode($this->isrc)), $this->id);
			if($this->info) $song_arr['info'] = apply_filters('musicstore_song_info', esc_url($this->info), $this->id);
			...
			if(!empty($this->file)){
                if($music_store_settings[ 'ms_paypal_enabled' ] && $music_store_settings[ 'ms_paypal_email' ] && !empty($this->price)){
					$price = music_store_apply_taxes($this->price);
					$song_arr['price'] = ((!empty($currency_symbol)) ? $currency_symbol.sprintf("%.2f", $price) : sprintf("%.2f", $price).$music_store_settings[ 'ms_paypal_currency' ]);
					if(
					...
					)
					{
						...
						$song_arr['salesbutton'] = apply_filters(
							...
							'<form action="'.esc_url(MS_H_URL).'" method="post"><input type="hidden" name="ms-action" value="buynow" /><input type="hidden" name="ms_product_type" value="single" /><input type="hidden" name="ms_product_id" value="'.esc_attr($this->id).'" /><input type="image" src="'.esc_url($paypal_button).'" style="padding-top:5px;" onclick="return ms_buy_now(this, '.$this->ID.');" alt="'.esc_attr__('Buy now', MS_TEXT_DOMAIN).'" /></form>',
							$this->id
						);
					}
                }elseif(
					(
						$music_store_settings['ms_hide_download_link_for_price_in_blank'] == false &&
						(
					...
					(
						$music_store_settings['ms_hide_download_link_for_price_in_blank'] == true &&
						$music_store_settings[ 'ms_download_link_for_registered_only' ] == true &&
						is_user_logged_in()
					)
				)
				{
                    $song_arr['salesbutton']  = apply_filters(
															...
															'<a href="'.esc_url($this->file).'" target="_blank" data-id="'.esc_attr($this->id).'" class="ms-download-link">'.__('Download', MS_TEXT_DOMAIN).'</a>'.( ( !empty( $music_store_settings[ 'ms_license_for_free' ] ) ) ? '|<a href="'.esc_url($music_store_settings[ 'ms_license_for_free' ]).'" target="_blank">'.__( 'License',  MS_TEXT_DOMAIN ).'</a>' : '' ),
															$this->id);
                }
			}
			...
			if($demo)
			{
                $song_arr['demo'] = apply_filters('musicstore_song_audio_tag', '<audio preload="none" data-product="'.$this->ID.'"><source src="'.$demo.'" type="audio/'.music_store_get_type( $demo ).'" /></audio>', $this->id);
			}else{
				$song_arr['demo'] = '';
			}
			...
			if($mode == 'store' || $mode == 'multiple'){
				...
				$demo = $this->demo;
				$song_arr['demo'] 			= ($demo) ? apply_filters('musicstore_song_audio_tag', '<audio style="width:100%;"  data-product="'.$this->ID.'"><source src="'.$demo.'" type="audio/'.music_store_get_type( $demo ).'" /></audio>', $this->id) : '';
				...
				if($music_store_settings['ms_pp_related_products'])
				{
					...
					{
						$related_products = '<section class="music-store-related-products"><h2>'.__('Related Products', MS_TEXT_DOMAIN).'</h2>'.$related_products.'</section>';
					}
					else
					{
						$related_products = '';
...
<?php
		...
		*/
		public static function save_data($post){
			...
			if ( empty($_POST['ms_song_box_content_nonce']) || !wp_verify_nonce( $_POST['ms_song_box_content_nonce'], plugin_basename( __FILE__ ) ) )
			return;
			...
			$data = array(
						...
						'isrc'      => sanitize_text_field($_POST['ms_isrc'])
					);
			...
			if(isset($_POST['ms_artist'])){
				if(is_array( $_POST['ms_artist'] ))
				{
					...
					{
						$artist = sanitize_text_field(stripcslashes($artist));
					...
					wp_set_object_terms($id, sanitize_text_field(stripcslashes($_POST['ms_artist'])), 'ms_artist', true);
			}
			...
			if(isset($_POST['ms_artist_new'])){
				if(is_array($_POST['ms_artist_new']))
				{
					...
					$_POST['ms_artist_new'] = array_map('sanitize_text_field',$_POST['ms_artist_new']);
				}
				else $_POST['ms_artist_new'] = sanitize_text_field(stripcslashes($_POST['ms_artist_new']));
			...
			if(isset($_POST['ms_album'])){
				...
				{
					$album = sanitize_text_field(stripcslashes($album));
			...
			if(isset($_POST['ms_album_new'])){
				if(is_array($_POST['ms_album_new']))
				{
					...
					$_POST['ms_album_new'] = array_map('sanitize_text_field',$_POST['ms_album_new']);
				}
				else $_POST['ms_album_new'] = sanitize_text_field(stripcslashes($_POST['ms_album_new']));
		...
		*/
		public static function columns($columns){
			return array(
				...
				'popularity'  => __('Popularity', MS_TEXT_DOMAIN),
